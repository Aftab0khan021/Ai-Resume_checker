// src/components/ui/sections/AnalysisDetailContent.jsx
...
const AnalysisDetailContent = ({ analysis }) => {
  ...
  const analysisSummary = safeString(analysis.analysis_summary);
  ...
  const resumeText = safeString(analysis.resume_text);
  const jobDescription = safeString(analysis.job_description);
      ...
      <div className="xl:col-span-2 space-y-6">
        <Card>
          ...
          <CardContent>
            ...
            <Progress value={matchPercentage} className="w-full" indicatorClassName={getProgressColor(matchPercentage)} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card>
            ...
            <CardContent>
              <div className={`text-5xl font-bold ${getMatchColor(atsScore)}`}>{atsScore.toFixed(0)}</div>
              <p className="text-slate-600 mt-2">A score above 80 is ideal for passing automated screening.</p>
            </CardContent>
          </Card>
      ...
      <div className="space-y-6">
          ...
          <CardContent className="flex flex-wrap gap-2">
            {matchedSkills.length > 0 ? (
              matchedSkills.map((skill, i) => (
                <Badge key={i} variant="success">{safeString(skill)}</Badge>
              ))
            ) : (
          ...
          <CardContent className="flex flex-wrap gap-2">
            {missingSkills.length > 0 ? (
              missingSkills.map((skill, i) => (
                <Badge key={i} variant="destructive">{safeString(skill)}</Badge>
              ))
            ) : (
        ...
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="resume">
            ...
            <AccordionContent>
              <ScrollArea className="h-64 p-4 border rounded-md bg-slate-50">
                <pre className="text-xs text-slate-700 whitespace-pre-wrap font-mono">{resumeText || "No resume text available."}</pre>
              </ScrollArea>
            </AccordionContent>
          </AccordionItem>
          ...
          <AccordionItem value="jd">
            ...
            <AccordionContent>
              <ScrollArea className="h-64 p-4 border rounded-md bg-slate-50">
                <pre className="text-xs text-slate-700 whitespace-pre-wrap font-mono">{jobDescription || "No job description provided."}</pre>
              </ScrollArea>
            </AccordionContent>
          </AccordionItem>
        </Accordion>
